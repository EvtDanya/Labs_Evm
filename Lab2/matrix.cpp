...
void dgemm(const double* a, const double* b, double* result, const int size) {
    int i, j, k;
...
void dgemm_opt1(double* a, double* b, double* result, const int size) {
    int i, j, k;
    // Выполняем умножение матриц
    for (i = 0; i < size; i++) {
        for (k = 0; k < size; k++) {
...
void dgemm_opt2(const double* a, const double* b, double* result, const int size, const int block_size) {
